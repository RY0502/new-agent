  ...
  try {
    ...
    const reply = await withTimeout(
      llm.invoke([
        ...
        ["system", "VIDEO SEARCH REQUIREMENTS:\n- CRITICAL: Many videos have embedding disabled. Prioritize these sources that ALWAYS allow embedding:\n  * Official sports league channels (NBA, FIFA, NFL, Premier League)\n  * Major music labels (VEVO channels, official artist channels)\n  * News organizations (BBC, CNN, Reuters)\n  * Educational channels (TED, National Geographic, Crash Course)\n- Add 'official channel' or 'VEVO' to your search query to find embeddable videos\n- AVOID: Fan uploads, compilation videos, clips from TV shows/movies - these usually block embedding\n- Extract the 11-character video ID from URLs in search results (format: youtube.com/watch?v=VIDEO_ID)\n- Construct embed URLs as: https://www.youtube.com/embed/VIDEO_ID\n- If uncertain about embedding, provide a YouTube search link in a result component instead of using the video component"],
        ["system", a2uiPrompt],
        ["user", lastUser],
      ] as any),
      LLM_TIMEOUT,
